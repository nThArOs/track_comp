...
class CocoToYoloCrowdHuman():
    def __init__(self, data_path: Path, out_path: Path=None, split="test", image_path=None, num_workers=8):
    ...
    def prepare_images(self):
        ...
        print(f"Nb video {len(self.data['videos'])}")
        print(self.data["videos"])
        print(f"Nb frame {len(self.data['images'])}")
        for img in tqdm(self.data['images']):
            filename = img['file_name']
            namefile = filename.split("/")
            newname = namefile[0] + "_" + namefile[1] + "_" + namefile[2]

            if os.path.exists(self.image_path / self.make_filename(img['file_name'])):
                self.file_names.append(filename)

            #print(f"copy {os.path.join(self.path_to_images, filename)}---->,{self.image_path},------->{namefile}")
            copy_and_rename(os.path.join(self.path_to_images, filename), self.image_path, newname)
            self.file_names.append(filename)
        print("Done!")
    ...
    def prepare_label(self, filename):
        if os.path.exists(f"{self.label_path}/{self.make_filename(filename).replace('png', 'txt')}"):
            return
        ...
        if img_ann:
            ...
            file_object = open(f"{self.label_path}/{self.make_filename(filename).replace('png', 'txt')}", "a")
...
if __name__ == '__main__':
    ...
    args.add_argument('--data_path',default="datasets/MOT17", type=str)
    args.add_argument('--image_path', type=str, default=None)
    args.add_argument('--split', nargs='+', default=["test", "train"])
    args.add_argument('--dataset', type=str, default="mot", help="crowdhuman or mot")
