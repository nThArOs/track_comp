...
def mot17_to_coco(DATA_PATH,img_format, OUT_PATH=None, SPLITS= ['train_half', 'val_half', 'train', 'test'], HALF_VIDEO=False, CREATE_SPLITTED_ANN=True, CREATE_SPLITTED_DET=True):
    if OUT_PATH is None:
    ...
    for split in SPLITS:        # iteration over split strategy
        ...
        for seq in tqdm(sorted(seqs)):
            print(sorted(seqs))
            #if 'mot' in DATA_PATH.lower() and (split != 'test' and not ('FRCNN' in seq)):
            #if 'mot' in DATA_PATH.lower() and (split != 'test' and not ('FRCNN' in seq)):
                #continue
            ...
            print(images)
            num_images = len([image for image in images if img_format in image])  # half and half
            print(num_images)
            ...
            for i in range(num_images):
                ...
                #wprint(os.path.join(data_path, '{}/img1/{:06d}.'.format(seq, i)+img_format))

                img = cv2.imread(os.path.join(data_path, '{}/img1/{:06d}.'.format(seq, i)+img_format))


                height, width = img.shape[:2]
                image_info = {'file_name': '{}/img1/{:06d}.'.format(seq, i)+img_format,  # image name.
            ...
            if split != 'test':
                ...
                print('{} an images'.format(int(anns[:, 0].max())))
...
if __name__ == '__main__':
    ...
    args.add_argument('--image_format', default="png", type=str)
    args.add_argument('--out_path', type=str, default=None)
    args = args.parse_args()
    print(args.data_path)
    print(args.image_format)
    mot17_to_coco(args.data_path,args.image_format)